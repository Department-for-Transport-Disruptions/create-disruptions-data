import { z } from "zod";
export const setZodDefaultError = (errorMessage) => ({
    errorMap: (issue) => {
        switch (issue.code) {
            default:
...
export const makeZodArray = (schema) => z.array(schema);
 ...
 */
export const zodDate = (defaultError) => z.string(defaultError ? setZodDefaultError(defaultError) : {}).regex(dateRegex);
/**
 ...
 */
export const zodTime = (defaultError) => z.string(defaultError ? setZodDefaultError(defaultError) : {}).regex(timeRegex);
/**
 ...
 */
export const zodTimeInMinutes = (defaultError) => z.string(defaultError ? setZodDefaultError(defaultError) : {}).regex(minutesRegex);
/**
 ...
 */
export const zodUuid = (defaultError) => z.string(defaultError ? setZodDefaultError(defaultError) : {}).regex(uuidRegex);
/**
 ...
 */
export const makeFilteredArraySchema = (schema) => z
    .array(z.unknown())
    .transform((items) => items?.filter((item) => schema.safeParse(item).success));
export const isValidTime = (time) => time.match(timeRegex);
