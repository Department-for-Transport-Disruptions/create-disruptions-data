...
export const getAllUsersInGroup = async (groupName, nextToken) => {
    const users = await cognito.send(new ListUsersInGroupCommand({ UserPoolId: userPoolId, GroupName: groupName, Limit: 60, NextToken: nextToken }));
    if (!users.Users) {
        return [];
    }
    if (users.NextToken) {
...
export const getAllUsersEmailsInGroups = async (groupNames, orgIds) => {
    const usersEmailsOrgIds = [];
...
export const getUsersByAttributeByOrgIds = async (attributeToHave, attributeToGet, attributeToHaveValue, orgIdsAndAdminAreaCodes) => {
    const params = {
