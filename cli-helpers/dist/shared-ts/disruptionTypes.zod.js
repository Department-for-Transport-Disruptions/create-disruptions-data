...
import { environmentReasonSchema, equipmentReasonSchema, miscellaneousReasonSchema, personnelReasonSchema, } from "./siriTypes.zod";
...
export const validitySchemaRefined = validitySchema
    ...
    if (!val.disruptionNoEndDateTime &&
        val.disruptionRepeats === "daily" &&
        val.disruptionRepeatsEndDate &&
        getFormattedDate(val.disruptionRepeatsEndDate).isSameOrAfter(getDatetimeFromDateAndTime(val.disruptionStartDate, val.disruptionStartTime).add(365, "day"))) {
        return false;
    }
    return true;
}, {
    ...
    if (!val.disruptionNoEndDateTime &&
        ...
        val.disruptionRepeats === "daily" &&
        getDatetimeFromDateAndTime(val.disruptionEndDate, val.disruptionEndTime).isSameOrAfter(getDatetimeFromDateAndTime(val.disruptionStartDate, val.disruptionStartTime).add(24, "hours"))) {
        return false;
    }
    return true;
}, {
    ...
    if (!val.disruptionNoEndDateTime &&
        ...
        val.disruptionEndTime &&
        val.disruptionRepeats === "weekly" &&
        getDatetimeFromDateAndTime(val.disruptionEndDate, val.disruptionEndTime).isSameOrAfter(getDatetimeFromDateAndTime(val.disruptionStartDate, val.disruptionStartTime).add(7, "day"))) {
        return false;
    }
    return true;
}, {
...
export const disruptionInfoSchema = z.object({
    id: z.string().uuid(),
    disruptionType: z.union([z.literal("planned"), z.literal("unplanned")], setZodDefaultError("Select a disruption type")),
    ...
    associatedLink: z
        ...
        .optional()
        .or(z.literal("")),
    disruptionReason: z.union([miscellaneousReasonSchema, environmentReasonSchema, personnelReasonSchema, equipmentReasonSchema], setZodDefaultError("Select a reason from the dropdown")),
...
export const disruptionInfoSchemaRefined = disruptionInfoSchema
    ...
    if (val.disruptionEndDate && val.disruptionEndTime && val.disruptionStartDate && val.disruptionStartTime) {
        return getDatetimeFromDateAndTime(val.disruptionEndDate, val.disruptionEndTime).isAfter(getDatetimeFromDateAndTime(val.disruptionStartDate, val.disruptionStartTime));
    }
    return true;
}, {
    path: ["disruptionEndDate"],
    ...
    if (val.publishEndDate && val.publishEndTime && val.publishStartDate && val.publishStartTime) {
        return getDatetimeFromDateAndTime(val.publishEndDate, val.publishEndTime).isAfter(getDatetimeFromDateAndTime(val.publishStartDate, val.publishStartTime));
    }
    return true;
}, {
    path: ["publishEndDate"],
    ...
    .refine((val) => {
    const { validity = [], disruptionStartDate, disruptionStartTime, disruptionEndDate, disruptionEndTime, disruptionRepeats, disruptionRepeatsEndDate, } = val;
    if (!val.disruptionNoEndDateTime &&
        (!val.disruptionStartDate ||
            !val.disruptionStartTime ||
            !val.disruptionEndDate ||
    ...
    for (let i = 0; i < combinedValidity.length; i++) {
        for (let j = i + 1; j < combinedValidity.length; j++) {
            ...
            checkOverlap(getDatetimeFromDateAndTime(combinedValidity[i].disruptionStartDate, combinedValidity[i].disruptionStartTime), combinedValidity[i].disruptionEndDate
                ? getDatetimeFromDateAndTime(combinedValidity[i].disruptionEndDate || "", combinedValidity[i].disruptionEndTime || "")
                : dayjs().add(100, "year"), getDatetimeFromDateAndTime(combinedValidity[j].disruptionStartDate, combinedValidity[j].disruptionStartTime), combinedValidity[j].disruptionEndDate
                ? getDatetimeFromDateAndTime(combinedValidity[j].disruptionEndDate || "", combinedValidity[j].disruptionEndTime || "")
                : dayjs().add(100, "year"))) {
                return false;
            }
        }
    ...
    .superRefine((val, ctx) => {
    const { validity = [], disruptionStartDate, disruptionStartTime, disruptionEndDate, disruptionEndTime, disruptionRepeats, disruptionRepeatsEndDate, publishStartDate, publishStartTime, publishEndDate, publishEndTime, } = val;
    const combinedValidity = [
        ...validity,
        {
            disruptionStartDate,
    ...
    if (!isValidTime(disruptionStartTime) ||
    ...
    const sortedValidity = combinedValidity.sort((a, b) => {
        return getDatetimeFromDateAndTime(a.disruptionStartDate, a.disruptionStartTime).isBefore(getDatetimeFromDateAndTime(b.disruptionStartDate, b.disruptionStartTime))
            ? -1
            : 1;
    });
    if (getDatetimeFromDateAndTime(publishStartDate, publishStartTime).isAfter(getDatetimeFromDateAndTime(sortedValidity[0].disruptionStartDate, sortedValidity[0].disruptionStartTime))) {
        ctx.addIssue({
    ...
    for (let i = 0; i < sortedValidity.length; i++) {
        if ((sortedValidity[i].disruptionRepeats === "daily" || sortedValidity[i].disruptionRepeats === "weekly") &&
            sortedValidity[i].disruptionRepeatsEndDate) {
            const repeatsEndDate = getFormattedDate(sortedValidity[i].disruptionRepeatsEndDate || "");
            if (repeatsEndDate.isAfter(maxEndDate))
...
export const refinedNetworkConsequenceSchema = networkConsequenceSchema.refine((item) => {
...
export const servicesConsequenceSchema = z.object({
    ...
    disruptionDirection: z.union([z.literal("allDirections"), z.literal("inbound"), z.literal("outbound")], setZodDefaultError("Select a direction")),
...
export const disruptionSchema = disruptionInfoSchemaRefined.and(z.object({
    consequences: z
