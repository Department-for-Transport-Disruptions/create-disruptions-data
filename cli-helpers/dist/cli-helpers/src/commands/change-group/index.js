import { AdminAddUserToGroupCommand, AdminListGroupsForUserCommand, AdminRemoveUserFromGroupCommand, CognitoIdentityProviderClient, ListUserPoolsCommand, ListUsersCommand, } from "@aws-sdk/client-cognito-identity-provider";
...
const getUsers = async (poolId, paginationToken) => {
...
export default class CreateUser extends Command {
    ...
    async run() {
        ...
        const existingGroups = await cognito.send(new AdminListGroupsForUserCommand({
            UserPoolId: poolId,
            Username: email,
        }));
