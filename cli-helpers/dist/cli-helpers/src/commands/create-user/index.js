import { AdminAddUserToGroupCommand, AdminCreateUserCommand, CognitoIdentityProviderClient, ListUserPoolsCommand, } from "@aws-sdk/client-cognito-identity-provider";
...
const listOperatorsForOrg = async (orgId, stage) => {
    let dbData = [];
...
export default class CreateUser extends Command {
    ...
    async run() {
        ...
        if (!poolId) {
            const userPoolsResult = await cognito.send(new ListUserPoolsCommand({
                MaxResults: 10,
            }));
        ...
        if (!orgId) {
            const orgsDb = await ddbDocClient.send(new ScanCommand({
        ...
        const url = stage === "prod" ? "https://disruption-data.dft.gov.uk" : `https://${stage}.cdd.dft-create-data.com`;
