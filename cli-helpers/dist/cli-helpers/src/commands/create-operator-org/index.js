...
export const createOperatorSubOrganisation = async (orgId, operatorName, nocCodes, stage) => {
...
export default class CreateOperatorOrg extends Command {
    ...
    async run() {
        ...
        if (orgId && name && nocCodes) {
            const id = await createOperatorSubOrganisation(orgId, name, nocCodes?.split(","), stage);
