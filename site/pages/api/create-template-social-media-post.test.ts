...
describe("create-template-social-media-post API", () => {
    ...
    it("should redirect to /review-template when all required inputs are passed and an image", async () => {
        const { req, res } = getMockRequestAndResponse({
            body: { ...previousCreateSocialMediaPostInformation },
            mockWriteHeadFn: writeHeadMock,
        });
    ...
    it("should redirect to /create-template-social-media-post when the image size is over 5MB", async () => {
        const { req, res } = getMockRequestAndResponse({
            body: { ...previousCreateSocialMediaPostInformation },
            mockWriteHeadFn: writeHeadMock,
        });
        ...
        expect(writeHeadMock).toBeCalledWith(302, {
            Location: `${CREATE_TEMPLATE_SOCIAL_MEDIA_POST_PAGE_PATH}/${defaultDisruptionId}/0`,
        });
    });

    it("should redirect to /create-template-social-media-post when the image is the wrong type", async () => {
        const { req, res } = getMockRequestAndResponse({
            body: previousCreateSocialMediaPostInformation,
            mockWriteHeadFn: writeHeadMock,
        });
    ...
    it("should redirect to /create-template-social-media-post when publish date is in the past", async () => {
        const { req, res } = getMockRequestAndResponse({
