...
import { COOKIES_DISRUPTION_ERRORS, DASHBOARD_PAGE_PATH, DISRUPTION_DETAIL_PAGE_PATH } from "../../constants";
...
describe("create-disruption API", () => {
    ...
    it("should redirect back to /create-disruption when invalid reason passed with the expected query param value", async () => {
        ...
        expect(writeHeadMock).toBeCalledWith(302, {
            Location: `/create-disruption/${defaultDisruptionId}?template=true`,
        });
    });

    it("should redirect to given returnPath when all required inputs are passed and the disruption has a return query param", async () => {
        const disruptionData = {
            ...defaultDisruptionData,
            publishStartTime: "0900",
            disruptionStartDate: getFutureDateAsString(40),
        ...
        const { req, res } = getMockRequestAndResponse({
            body: disruptionData,
            mockWriteHeadFn: writeHeadMock,
            query: { return: `${DISRUPTION_DETAIL_PAGE_PATH}/${defaultDisruptionId}` },
        });
