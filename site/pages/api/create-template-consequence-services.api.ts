...
const createTemplateConsequenceServices = async (req: NextApiRequest, res: NextApiResponse): Promise<void> => {
    try {
        ...
        if (!validatedBody.success) {
            ...
            redirectTo(
                res,
                `${CREATE_TEMPLATE_CONSEQUENCE_SERVICES_PATH}/${body.disruptionId}/${body.consequenceIndex}`,
            );
            return;
        }

        const template = await upsertConsequence(validatedBody.data, session.orgId, session.isOrgStaff, true);
        destroyCookieOnResponseObject(COOKIES_TEMPLATE_CONSEQUENCE_SERVICES_ERRORS, res);
        ...
        if (e instanceof Error) {
            const message = "There was a problem adding a consequence services template.";
            redirectToError(res, message, "api.create-template-consequence-services", e);
            return;
        }
