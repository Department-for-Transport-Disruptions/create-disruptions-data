...
import duplicateConsequenceTemplate from "./duplicate-consequence-template.api";
import { ERROR_PATH, REVIEW_TEMPLATE_PAGE_PATH, TEMPLATE_OVERVIEW_PAGE_PATH } from "../../constants";
...
describe("duplicate-consequence API", () => {
    ...
    it("should redirect to /review-template when all required inputs are passed", async () => {
        ...
        getTemplateByIdSpy.mockResolvedValue({ ...disruption, publishStatus: PublishStatus.draft });
        await duplicateConsequenceTemplate(req, res);
    ...
    it("should redirect to /template-overview when all required inputs are passed", async () => {
        const { req, res } = getMockRequestAndResponse({
            body: { disruptionId: defaultDisruptionId },
            query: { consequenceId: defaultConsequenceIndex },
            mockWriteHeadFn: writeHeadMock,
    ...
    it("should redirect to /500 when disruptionId is missing", async () => {
        ...
        expect(getTemplateByIdSpy).not.toHaveBeenCalled();
    ...
    it("should redirect to /500 when getTemplateById does not return a disruption", async () => {
        const { req, res } = getMockRequestAndResponse({
            body: { disruptionId: defaultDisruptionId },
            query: { consequenceId: defaultConsequenceIndex },
            mockWriteHeadFn: writeHeadMock,
        ...
        getTemplateByIdSpy.mockResolvedValue(null);
        await duplicateConsequenceTemplate(req, res);
