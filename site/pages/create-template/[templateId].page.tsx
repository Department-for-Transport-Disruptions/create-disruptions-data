...
import {
    disruptionInfoSchema as templateInfoSchema,
    validitySchema,
...
const title = "Create Templates";
const description = "Create Templates page for the Create Transport Disruptions Service";

export interface TemplatePageProps extends PageState<Partial<DisruptionInfo>> {
...
const CreateTemplate = (props: TemplatePageProps): ReactElement => {
    ...
    const returnPath =
        props.disruptionStatus === PublishStatus.draft ? REVIEW_TEMPLATE_PAGE_PATH : TEMPLATE_OVERVIEW_PAGE_PATH;
    ...
    return (
        <BaseLayout title={title} description={description} errors={props.errors}>
            <CsrfForm action="/api/create-template" method="post" csrfToken={props.csrfToken}>
                <>
                    ...
                    {!props.templateExists ? (
                        <Link
                            role="button"
                            href={VIEW_ALL_TEMPLATES_PAGE_PATH}
                            className="govuk-button ml-5 govuk-button--secondary"
                        >
                            Cancel
                        </Link>
...
export const getServerSideProps = async (ctx: NextPageContext): Promise<{ props: TemplatePageProps }> => {
    const cookies = parseCookies(ctx);
    const errorCookie = cookies[COOKIES_TEMPLATE_ERRORS];
    ...
    const templateId = ctx.query.templateId?.toString() ?? "";
    const template = await getTemplateById(templateId, session.orgId);
    ...
    if (!template) {
        return {
            props: {
                ...getPageState(errorCookie, templateInfoSchema, templateId),
                consequenceIndex: 0,
            },
        };
    }
    ...
    return {
        props: {
            ...getPageState(errorCookie, templateInfoSchema, template.disruptionId, template),
            templateExists: true,
            consequenceIndex: template.consequences?.[0]?.consequenceIndex ?? 0,
            disruptionId: template.disruptionId,
