...
const approvalEmailBody = (
    ...
    disruptionCreator: string,
    disruptionLink: string,
) =>
    `<!DOCTYPE html> <html lang="en"><head><title>Create Transport Disruptions Data service</title></head>
                       ...
                       <h1>Approval Required - Create Transport Disruptions Data service</h1>
                       <p>A disruption has been created that requires your approval before it can be published.</p>
                       <p>Disruption Summary: ${disruptionSummary}</p>
                       <p>Disruption Description: ${disruptionDescription}</p>
                       <p>Submitted by: ${disruptionCreator}</p>
                       <a href=${disruptionLink}>Link to disruption</a>
                    </div>
                                 </div>
                               </div>
                             </body> 
...
export const createDisruptionApprovalEmail = (
    ...
    return new SendEmailCommand({
        Destination: {
            ToAddresses: orgAdminEmails,
        },
        Message: {
            Body: {
                Html: {
                    Charset: "UTF-8",
                    Data: approvalEmailBody(
                        disruptionSummary,
