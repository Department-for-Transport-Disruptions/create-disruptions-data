...
from txc_processor import (check_file_has_usable_data, collect_journey_pattern_section_refs_and_info,
                           collect_journey_patterns, collect_track_data, collect_vehicle_journey, create_unique_line_id,
                           download_from_s3_and_write_to_db, extract_data_for_txc_operator_service_table,
                           format_vehicle_journeys, iterate_through_journey_patterns_and_run_insert_queries, make_list,
                           select_route_and_run_insert_query, is_service_operational, calculate_days_of_operation)
...
class TestCalculateDaysOfOperation:
    @pytest.mark.parametrize("days_of_week, expected_result", [({"MondayToFriday": None},
...
class TestJourneyIsOperational:
    @pytest.mark.parametrize("vehicle_journey, expected_result",
    ...
    def test_journey_is_operational_using_vehicle_journey(self, vehicle_journey, expected_result):
        ...
        assert is_service_operational(vehicle_journey, [], service_operating_profile, service_operating_period,
                                      datetime.date(2025, 5, 8)) == expected_result

    @pytest.mark.parametrize("service_operating_profile, expected_result",
                             [(test_data.generate_mock_operating_profile({"MondayToFriday": None}), True),
                              ...
                              (None, True)], )
    def test_journey_is_operational_using_operating_profile(self, service_operating_profile, expected_result):
    ...
    def test_journey_is_operational_using_operating_period(self):
        ...
        assert is_service_operational(vehicle_journey, [], service_operating_profile, service_operating_period,
                                      datetime.date(2025, 5, 8)) == False

    @pytest.mark.parametrize("vehicle_journey, bank_holidays, expected_result", [(
    test_data.generate_mock_journey_bank_holiday({'DaysOfNonOperation': {'AllBankHolidays': None}}),
    [{"title": "Mock bank holiday", "date": "2025-05-08", }], False), (
            test_data.generate_mock_journey_bank_holiday({'DaysOfOperation': {'AllBankHolidays': None}}),
            [{"title": "Mock bank holiday", "date": "2025-05-08", }], True), (
    test_data.generate_mock_journey_bank_holiday({'DaysOfNonOperation': {'SpringBank': None}}),
    [{"title": "Spring bank holiday", "date": "2025-05-08", }], False), (
            test_data.generate_mock_journey_bank_holiday({'DaysOfOperation': {'SpringBank': None}}),
            [{"title": "Spring bank holiday", "date": "2025-05-08", }], True)])
    def test_journey_is_operational_bank_holiday(self, vehicle_journey, bank_holidays, expected_result):
        service_operating_profile = None
        service_operating_period = mock_data_dict["TransXChange"]["Services"]["Service"]["OperatingPeriod"]
        assert is_service_operational(vehicle_journey, bank_holidays, service_operating_profile,
                                      service_operating_period, datetime.date(2025, 5, 8)) == expected_result
...
class TestDatabaseInsertQuerying:
    ...
    def test_insert_methods_are_called_correct_number_of_times(self, mock_jp_insert, mock_jpl_insert):
        ...
        vehicle_journeys, _ = format_vehicle_journeys(
            mock_data_dict["TransXChange"]["VehicleJourneys"]["VehicleJourney"], "l_4_ANW", [], service)
        ...
        iterate_through_journey_patterns_and_run_insert_queries(mock_cursor, mock_data_dict, mock_op_service_id,
                                                                service, vehicle_journeys, "JP4", logger, )
...
class TestDataCollectionFunctionality:
    def test_extract_data_for_txc_operator_service_table(self):
        expected_operator_and_service_info = (
            "ANWE", "2018-01-28", "2099-12-31", "ANW", "The Pike - Evesham Country Park",
            "Evesham Country Park - The Pike", "Macclesfield - Upton Priory Circular", "NW_01_ANW_4_1", "Macclesfield",
            "Macclesfield", "bus",)
        operator = mock_data_dict["TransXChange"]["Operators"]["Operator"]
        service = mock_data_dict["TransXChange"]["Services"]["Service"]
        line = service["Lines"]["Line"]

        assert (extract_data_for_txc_operator_service_table(operator, service,
                                                            line) == expected_operator_and_service_info)

    def test_extract_data_for_non_bus_txc_operator_service_table(self):
        expected_operator_and_service_info = (
            "NXSF", "2022-07-25", "2023-02-02", "NEXUS Ferry", "", "", "South Shields - North Shields",
            "NE_04_FER_FERR_1", "South Shields", "North Shields", "ferry",)
    ...
    def test_collect_journey_pattern_section_refs_and_info(self):
        mock_raw_journey_patterns = mock_data_dict["TransXChange"]["Services"]["Service"]["StandardService"][
            "JourneyPattern"]
    ...
    def test_collect_journey_patterns(self):
        service = mock_data_dict["TransXChange"]["Services"]["Service"]
        assert (collect_journey_patterns(mock_data_dict, service) == test_data.expected_list_of_journey_patterns)
    ...
    def test_collect_vehicle_journey(self):
        ...
        assert (collect_vehicle_journey(mock_data_dict["TransXChange"]["VehicleJourneys"]["VehicleJourney"][0], [],
                                        mock_service_operating_profile,
                                        mock_service_operating_period) == test_data.expected_vehicle_journey)

    @patch("txc_processor.insert_into_txc_journey_pattern_table")
    @patch("txc_processor.insert_into_txc_journey_pattern_link_table")
    def test_correct_route_ref_and_links_selected_for_journey_pattern(self, mock_jp_insert, mock_jpl_insert):
        service = mock_tracks_data_dict["TransXChange"]["Services"]["Service"]
        mock_cursor = MagicMock()
        mock_op_service_id = 12
        vehicle_journeys, _ = format_vehicle_journeys(
            mock_data_dict["TransXChange"]["VehicleJourneys"]["VehicleJourney"], "l_4_ANW", [], service)
        route_ref, link_refs = iterate_through_journey_patterns_and_run_insert_queries(mock_cursor,
                                                                                       mock_tracks_data_dict,
                                                                                       mock_op_service_id, service,
                                                                                       vehicle_journeys, "JP1",
                                                                                       logger, )

        assert route_ref == "RT1"
        assert link_refs == ["RL1", "RL2", "RL3", "RL4", "RL5", "RL6", "RL7", "RL8", "RL9", "RL10", "RL11", "RL12",
                             "RL13", ]

    @patch("txc_processor.insert_into_txc_tracks_table")
    def test_correct_tracks_are_inserted_with_multiple_route_sections(self, mock_routes_insert):
        mock_cursor = MagicMock(spec=cursor)
        mock_op_service_id = 12
        select_route_and_run_insert_query(mock_cursor, mock_tracks_data_dict, mock_op_service_id, "RT1",
                                          ["RL1", "RL2", "RL3", "RL4", "RL5", "RL6", "RL7", "RL8", "RL9", "RL10",
                                           "RL11", "RL12", "RL13", ], )

        mock_routes_insert.assert_called_once_with(mock_cursor, test_data.expected_tracks_data_multiple_sections,
                                                   mock_op_service_id, )

    @patch("txc_processor.insert_into_txc_tracks_table")
    def test_correct_tracks_are_inserted_with_one_route_section(self, mock_routes_insert):
        mock_cursor = MagicMock(spec=cursor)
        mock_op_service_id = 12
        select_route_and_run_insert_query(mock_cursor, mock_tracks_data_dict, mock_op_service_id, "RT3", ["RL14"], )

        mock_routes_insert.assert_called_once_with(mock_cursor, test_data.expected_tracks_data_single_section,
                                                   mock_op_service_id, )
...
class TestMainFunctionality:
    ...
    def test_integration_between_s3_download_and_database_write_functionality(self, db_patch, s3, cloudwatch):
        ...
        download_from_s3_and_write_to_db(s3, cloudwatch, mock_bucket, mock_key, mock_file_dir, db_connection, logger,
                                         {})
        db_patch.assert_called_once_with(mock_data_dict, "WM", "tnds", mock_key, db_connection, logger, cloudwatch, {})
